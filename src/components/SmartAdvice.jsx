...
export default function SmartAdvice({ alerts, relays, onAcceptAdvice, activeScheduleId, onApplySchedule, visible, onClose, tokenKwhRemaining, targetHours, tokenState }) {
      ...
      <aside className="advice-panel slide-in" onClick={e => e.stopPropagation()} style={{ width: '480px', maxWidth: '95vw', background: 'rgba(18, 25, 29, 0.96)', backdropFilter: 'blur(30px)', borderLeft: '1px solid rgba(255,255,255,0.08)', boxShadow: '-10px 0 30px rgba(0,0,0,0.5)' }}>
        ...
        <div className="advice-list" style={{ padding: '24px', overflowY: 'auto', height: 'calc(100vh - 85px)' }}>
          ...
          {pendingScheduleActivation ? (() => {
            const recipe = allocation.recipes.find(r => r.id === pendingScheduleActivation);
            if (!recipe) return null;
            return (
              <div className="schedule-activation-prompt fade-in">
                <button 
                  onClick={() => setPendingScheduleActivation(null)}
                  style={{ background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontSize: '12px', padding: 0, marginBottom: '20px' }}
                >
                ...
                <div style={{ background: 'rgba(14, 165, 233, 0.1)', border: '1px solid rgba(14, 165, 233, 0.3)', borderRadius: '10px', padding: '20px', textAlign: 'center' }}>
                  <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="var(--accent-blue)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ marginBottom: 12 }}>
                    <circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" />
                  </svg>
                  <h4 style={{ fontSize: '16px', color: '#fff', margin: '0 0 12px 0' }}>Schedule Execution Mode</h4>
                  <p style={{ fontSize: '13px', color: 'var(--text-secondary)', lineHeight: '1.6', marginBottom: '24px', textAlign: 'left' }}>
                    The <strong>{recipe.name}</strong> schedule contains time-based rules (e.g., specific running hours). Do you want ZET-5 to actively track the time and switch the relays for you automatically, or do you prefer to apply these settings manually?
                  </p>

                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    <button 
                      onClick={() => onApplySchedule(recipe.id, true)}
                      className="advice-accept-btn"
                      style={{ padding: '12px', fontSize: '13px', fontWeight: 'bold' }}
                    >
                      Fully Autonomous (Recommended)
                    </button>
                    <button 
                      onClick={() => onApplySchedule(recipe.id, false)}
            ...
            const recipe = allocation.recipes.find(r => r.id === selectedRecipeDetails);
            if (!recipe) return null;
            return (
              <div className="schedule-details-view fade-in">
                <button 
                  onClick={() => setSelectedRecipeDetails(null)}
                  style={{ background: 'none', border: 'none', color: 'var(--accent-blue)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontSize: '12px', padding: 0, marginBottom: '20px', fontWeight: 'bold' }}
                >
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                    <polyline points="15 18 9 12 15 6" />
                  </svg>
                ...
                <h4 style={{ fontSize: '16px', color: '#fff', margin: '0 0 12px 0' }}>{recipe.name} Allocation</h4>
                <p style={{ fontSize: '12px', color: 'var(--text-secondary)', lineHeight: '1.5', marginBottom: '24px' }}>
                  Below is the exact component-level schedule applied under this plan. It maintains your <strong>{recipe.dailyBudget} kWh/day</strong> budget to successfully hit your target runway.
                </p>

                <div className="schedules-list" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {recipe.schedules.map((sched, sidx) => (
                    <div key={sidx} style={{ background: 'rgba(255,255,255,0.02)', padding: '12px 14px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.06)' }}>
                      <div style={{ color: 'var(--accent-blue)', fontWeight: 'bold', fontSize: '13px', marginBottom: 6 }}>{sched.appliance}</div>
                      <div style={{ color: 'rgba(255,255,255,0.85)', fontSize: '12px', lineHeight: '1.5' }}>
                        {sched.schedule}
                      </div>
                      {sched.appliance === "Geyser Loop" && kwh >= minGeyser.kwhRequired && (
                        <div style={{ fontSize: '10.5px', color: 'var(--warning-amber)', marginTop: 8, background: 'rgba(255,179,0,0.1)', padding: '6px 8px', borderRadius: '4px', border: '1px solid rgba(255,179,0,0.2)' }}>
            ...
            <>
              ...
              {allocation.recipes.filter(recipe => {
                if (tokenState?.isEmergency) {
                  // In Critical Emergency, ONLY show Survival Mode
                  return recipe.id === 'survival';
                } else if (tokenState?.belowThreshold || tokenState?.atRisk) {
                  // In Low Energy (Threshold), hide Comfort Mode
                  return recipe.id !== 'comfort';
                }
                // Nominal state: Show all
                return true;
              }).map((recipe) => {
                  ...
                  <div key={recipe.id} className="recipe-card" style={cardStyle}>
                    ...
                    <div style={{ padding: '14px 16px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                        <h5 style={{ fontSize: '13px', fontWeight: 'bold', color: '#fff', margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
                          {recipe.name}
                          {activeScheduleId === recipe.id && (
                            <span style={{ fontSize: '9px', backgroundColor: 'var(--accent-blue)', color: '#fff', padding: '2px 6px', borderRadius: '4px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Active</span>
                          )}
                        </h5>
                      ...
                      <div style={{ marginTop: '16px', display: 'flex', gap: '10px' }}>
                        <button 
                          onClick={() => setSelectedRecipeDetails(recipe.id)}
                          className="btn-secondary"
                          style={{ width: '100%', padding: '8px', fontSize: '11px', cursor: 'pointer', fontWeight: 'bold' }}
                        >
                      ...
                      <div style={{ marginTop: '10px', paddingBottom: '2px', display: 'flex', gap: '10px', flexDirection: 'column' }}>
                        {pendingScheduleActivation === recipe.id ? (
                          <div style={{ background: 'rgba(0,0,0,0.2)', padding: '12px', borderRadius: '8px', border: '1px solid var(--accent-blue)' }}>
                            {activeScheduleId && activeScheduleId !== recipe.id && (
                              <div style={{ color: 'var(--warning-amber)', fontSize: '11px', marginBottom: '8px', fontWeight: 'bold' }}>
                                ⚠️ Another schedule ({activeScheduleId.toUpperCase()}) is currently running. Do you wish to switch?
                              </div>
                            )}
                            <p style={{ margin: '0 0 10px 0', fontSize: '11px', color: '#fff' }}>How would you like to apply this recipe?</p>
                            <div style={{ display: 'flex', gap: '8px' }}>
                              <button
                                ...
                                className="advice-accept-btn"
                                style={{ flex: 1, padding: '6px', fontSize: '11px', cursor: 'pointer', textAlign: 'center' }}
                              >
                                Autonomous Mode
                              </button>
                              <button
                            ...
                            <button
                              onClick={() => setPendingScheduleActivation(null)}
                              style={{ width: '100%', marginTop: '8px', padding: '4px', fontSize: '10px', background: 'transparent', color: 'var(--text-muted)', border: 'none', cursor: 'pointer' }}
                            >
                              Cancel
                            </button>
                          </div>
                          ...
                          <button
                            ...
                            className="btn-secondary"
                            style={{ width: '100%', padding: '6px 12px', fontSize: '11px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, borderColor: 'var(--alert-red)', color: 'var(--alert-red)' }}
                          >
                            Deactivate Schedule
                          </button>
                        ) : (
