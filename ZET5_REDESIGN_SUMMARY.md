# ZET-5 — Systems Architecture & Presentation Instrument

> **Project Title:** ZET-5 (Zimbabwean Energy Tracker)
...
The ZET-5 is a closed-loop, edge-to-cloud IoT energy management system designed specifically for the **Zimbabwean residential energy context (ZESA prepaid tokens)**. 
...
### The ZET-5 Solution
ZET-5 solves this through three integrated layers:
...
└───────────┼─────────────────────────────────────────────────────────────────┘
            │
            │ Wireless Client AP Handshake (SSID: ZET-5)
            │
┌───────────▼─────────────────────────────────────────────────────────────────┐
│                          ZET-5 APPLICATION LAYER                            │
│                                                                             │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
...
*   **The Engineering Solution:** ZET-5 implements a **Hybrid Reconciliation Loop**. The user periodically types their actual meter reading into the dashboard. ZET-5 computes the cumulative drift and applies an **exponentially smoothed correction factor**:
...
*   This prevents ZET-5 from overestimating appliance usage.
...
```text
Act as a Principal Embedded Systems and IoT Engineer. I am working on my school research project called ZET-5 (Zimbabwean Energy Tracker). 

The system consists of:
1. An ESP32 DevKit V1 serving as an AP (SSID: ZET-5) running a WebSocket server on ws://192.168.4.1/ws. It uses EmonLib for current metrology and broadcasts JSON telemetry: { "sensors": [s1,s2,s3,s4,s5], "relays": [r1..r8] }. It accepts incoming toggle JSON commands: { "relay": index, "state": bool }. It has a HIL (Hardware-in-the-Loop) simulator flag `#define SIMULATE_SENSORS 1` to model appliances in C++ and drop their currents to 0.0A when their corresponding relays are switched off.
